...
> 适用版本：v1.1.0（当前主线，SQLite + PHP 8.1+）  
> 目标读者：有 PHP/HTML/CSS 基础的模板开发者  
> 配套文档：`Documents/系统架构文档.md`、`Documents/后台使用指南.md`
...
└── my-theme/
    ...
    ├── style.css           ← 主题样式 + 主题元数据（必须）
    └── screenshot.jpg      ← 后台主题封面图（推荐）
```
...
> **后台上传校验说明**：  
> 后台「外观区块 → 网站模版」会读取 `/themes` 目录中的主题，并使用 `screenshot.jpg` 作为封面图。  
> 若缺少 `screenshot.jpg`，后台会显示“无预览图片”；若缺少 `style.css` 或必需模板文件，则主题不可启用。

`style.css` 顶部必须包含主题元数据头部注释，至少要有 `Theme Name`：
...
| `company_phone` | 联系电话 | 后台「联系方式」 |
| `company_email` | 邮箱 | 后台「联系方式」 |
| `company_address` | 地址 | 后台「联系方式」 |
| `whatsapp` | WhatsApp（号码或完整链接） | 后台「联系方式」 |
| `facebook` / `instagram` / `linkedin` / `youtube` / `twitter` | 社交媒体链接 | 后台「联系方式」 |
| `company_bio` | 公司简介 | 后台「公司简介」 |
| `company_response_time` | 平均响应时间 | 后台「公司简介」 |
| `company_main_markets` | 主要市场 | 后台「贸易能力」 |
| `seo_title` / `seo_description` / `seo_keywords` | 全局 SEO | 后台「基础设置」 |
| `theme` | 当前主题名 | 后台「外观区块 → 网站模版」或「基础设置」 |
...
**产品项字段**：`id`, `title`, `slug`, `summary`, `cover`, `status`, `url`  
...
return [
    ...
    // 区块键（用于 block() 调用）
    'my_section' => [
        'group'       => 'home',        // 后台分组 key（推荐）
        'group_label' => '首页',         // 后台分组显示名（推荐）
        'label'       => '首页 - 我的区块',   // 后台显示名
...
建议为每个区块都补充 `group` 和 `group_label`。  
后台「外观区块 → 模板区块」现在会先按页面分组展示，再显示该分组下的具体区块，这样运营人员更容易定位内容。
...
后台菜单标识符仅支持**小写字母、数字和连字符 `-`**。  
保存时系统会自动过滤非法字符并转为小写，建议始终使用语义化命名，例如 `main-nav`、`footer-links`、`mobile-shortcuts`。
...
进入后台「外观区块 → 网站模版」，找到 `my-theme` 后点击“设为当前主题”。  
如果主题还未放入服务器，也可以直接上传 zip 包，系统会自动校验并解压到 `/themes` 目录。
...
1. 后台「网站模版」页面是否正确显示 `screenshot.jpg` 预览图。
2. `style.css` 头部元数据是否被正确识别（主题名、作者、版本、描述）。
3. 后台「模板区块」是否已经按预期分组显示当前主题区块。
4. 前台主要页面：首页、产品列表、产品详情、文章列表、联系页、关于页是否都能正常打开。
...
- 确认修改的是正确主题目录（后台「外观区块 → 网站模版」确认当前主题名）
...
| GET | `/blog` | `SiteController::blog` | `post_list.php` |
| GET | `/blog/:slug` | `SiteController::blogDetail` | `post_detail.php` |
...
| POST | `/contact` | `SiteController::contact` | `thanks.php` |
| POST | `/inquiry` | `SiteController::inquiry` | `thanks.php` |
...
*文档版本：2026-06-09 | 项目：shopagg B2B 企业官网模板*
