...
class CaseModel extends BaseModel {
    ...
    public function create(array $data): void {
        $stmt = $this->db->prepare("INSERT INTO cases (title, slug, summary, content, created_at, updated_at) VALUES (:t, :s, :sum, :c, :ca, :ua)");
    ...
    public function update(int $id, array $data): void {
        $stmt = $this->db->prepare("UPDATE cases SET title=:t, slug=:s, summary=:sum, content=:c, updated_at=:ua WHERE id=:id");
        $stmt->bindValue(':t', $data['title']);
        $stmt->bindValue(':s', $data['slug']);
        $stmt->bindValue(':sum', $data['summary']);
        $stmt->bindValue(':c', $data['content']);
