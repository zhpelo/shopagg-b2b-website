...
final class PluginRegistry {
    ...
    public function find(string $pluginId): ?array {
        $stmt = $this->prepare('SELECT * FROM plugins WHERE plugin_id = :id LIMIT 1');
    ...
    public function save(array $manifest, string $source = 'private', string $status = 'installed'): void {
        $now = gmdate('c');
        $stmt = $this->prepare("INSERT INTO plugins (
    ...
    public function setStatus(string $pluginId, string $status, ?string $activeVersion = null): void {
        $stmt = $this->prepare("UPDATE plugins SET status=:status, active_version=:version,
    ...
    public function recordFailure(string $pluginId, \Throwable $error): int {
        ...
        $stmt = $this->prepare('UPDATE plugins SET failure_count=failure_count+1,last_error=:error,updated_at=:now WHERE plugin_id=:id');
        ...
        if ($count >= 3) {
            $stmt = $this->prepare("UPDATE plugins SET status='error',active_version=NULL WHERE plugin_id=:id");
            $stmt->bindValue(':id', $pluginId, SQLITE3_TEXT);
            $stmt->execute();
    ...
    public function delete(string $pluginId): void {
        ...
        try {
            foreach (['plugin_job_runs', 'plugin_jobs', 'plugin_logs', 'plugin_migrations', 'plugin_options'] as $table) {
                $stmt = $this->prepare("DELETE FROM {$table} WHERE plugin_id=:id");
                $stmt->bindValue(':id', $pluginId, SQLITE3_TEXT);
                $stmt->execute();
            }
            $stmt = $this->prepare('DELETE FROM plugins WHERE plugin_id=:id');
            $stmt->bindValue(':id', $pluginId, SQLITE3_TEXT);
            $stmt->execute();
            $this->db->exec('COMMIT');
        } catch (\Throwable $e) {
    ...
    public function option(string $pluginId, string $key, mixed $default = null): mixed {
        $stmt = $this->prepare('SELECT option_value FROM plugin_options WHERE plugin_id=:id AND option_key=:key');
        $stmt->bindValue(':id', $pluginId, SQLITE3_TEXT);
    ...
    public function setOption(string $pluginId, string $key, mixed $value, bool $autoload = false): void {
        $stmt = $this->prepare("INSERT INTO plugin_options(plugin_id,option_key,option_value,autoload,updated_at)
    ...
    public function deleteOption(string $pluginId, string $key): void {
        $stmt = $this->prepare('DELETE FROM plugin_options WHERE plugin_id=:id AND option_key=:key');
        $stmt->bindValue(':id', $pluginId, SQLITE3_TEXT);
        $stmt->bindValue(':key', $key, SQLITE3_TEXT);
        $stmt->execute();
    }
    ...
    public function deleteOptionsByPrefix(string $pluginId, string $prefix): void {
        $stmt = $this->prepare("DELETE FROM plugin_options WHERE plugin_id=:id AND option_key LIKE :prefix ESCAPE '\\'");
        $stmt->bindValue(':id', $pluginId, SQLITE3_TEXT);
    ...
    public function log(string $pluginId, string $level, string $message, array $context = []): void {
        $stmt = $this->prepare('INSERT INTO plugin_logs(plugin_id,level,message,context_json,created_at) VALUES(:id,:level,:message,:context,:now)');
        $stmt->bindValue(':id', $pluginId, SQLITE3_TEXT);
    ...
    public function logs(string $pluginId = '', int $limit = 100): array {
        ...
        if ($pluginId !== '') {
            $stmt = $this->prepare("SELECT * FROM plugin_logs WHERE plugin_id=:id ORDER BY id DESC LIMIT {$limit}");
            $stmt->bindValue(':id', $pluginId, SQLITE3_TEXT);
            $result = $stmt->execute();
        } else {
    ...
    private function prepare(string $sql): SQLite3Stmt {
        ...
        if ($stmt === false) {
            throw new \RuntimeException('插件注册表数据库语句准备失败：' . $this->db->lastErrorMsg());
        }
        return $stmt;
    }
}
