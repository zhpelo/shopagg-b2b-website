    ...
    <div class="border-b border-slate-200 bg-indigo-600 p-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-indigo-100 mt-1">管理网站首页及各页面的轮播图区块</p>
            </div>
            <a href="<?= url('/admin/appearance/sliders/create') ?>" 
    ...
    <div class="p-6">
        <?php if (empty($sliders)): ?>
            ...
            <div class="text-center py-16">
                ...
                <h3 class="text-lg font-semibold text-slate-900 mb-2">暂无轮播图</h3>
                <p class="text-slate-500 mb-6">还没有创建任何轮播图区块</p>
                <a href="<?= url('/admin/appearance/sliders/create') ?>" 
                    ...
                    <div class="group border border-slate-200 rounded-xl hover:shadow-lg transition-all duration-300 bg-white overflow-hidden">
                        ...
                        <div class="p-5 border-b border-slate-100 bg-slate-50/50">
                            <div class="flex items-start justify-between gap-4">
                                ...
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium <?= $slider['status'] === 'active' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600' ?>">
                                    <?= $slider['status'] === 'active' ? '启用' : '禁用' ?>
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="px-5 py-4 border-t border-slate-100 bg-slate-50/30 flex items-center justify-between">
                            <span class="text-xs text-slate-400">
                                更新于 <?= format_date($slider['updated_at']) ?>
                            </span>
                            <div class="flex items-center gap-2">
                                <a href="<?= url('/admin/appearance/sliders/edit?id=' . $slider['id']) ?>" 
                                ...
                                <button type="button"
                                        onclick="confirmDelete(<?= $slider['id'] ?>, '<?= h($slider['name']) ?>')"
    ...
    <div class="relative bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
        <div class="text-center">
            ...
            <p class="text-slate-600 mb-6">
                确定要删除轮播图「<span id="delete-slider-name" class="font-medium text-slate-900"></span>」吗？<br>
                <span class="text-rose-500 text-sm">此操作不可恢复，关联的所有图片也将被删除。</span>
            </p>
            <div class="flex items-center justify-center gap-3">
                ...
                <button type="button" id="delete-confirm-btn" data-post-action="<?= url('/admin/appearance/sliders/delete') ?>" data-post-payload="{}"
...
function confirmDelete(id, name) {
    document.getElementById('delete-slider-name').textContent = name;
