...
$returnTo = '/admin/app-store/themes/' . $themeId;
...
<div class="space-y-6">
    ...
    <?php if (!empty($appStore['wechat_pay'])): ?>
        <div class="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800">
            ...
            <p class="mt-3 break-all rounded-xl bg-white px-3 py-2 font-mono text-xs text-emerald-900"><?= h($appStore['wechat_pay']['code_url'] ?? '') ?></p>
        </div>
    <?php endif; ?>
    ...
    <div class="grid gap-6 xl:grid-cols-12">
        <div class="space-y-6 xl:col-span-8">
            ...
            <div class="rounded-xl border border-slate-200 bg-white p-6">
                ...
                <div class="rich-content rounded-xl border border-slate-200 bg-slate-50 p-5">
            ...
            <div class="rounded-xl border border-slate-200 bg-white p-6">
                ...
                <?php elseif ($changelog !== ''): ?>
                    <div class="rounded-xl border border-slate-200 bg-slate-50 p-5 text-sm leading-7 text-slate-600">
                        <?= nl2br(h($changelog)) ?>
                    </div>
                <?php else: ?>
                    <div class="rounded-xl border border-slate-200 bg-slate-50 p-5 text-sm text-slate-500">
                        暂无版本记录。
                    </div>
                <?php endif; ?>
            </div>
        ...
        <aside class="space-y-6 xl:col-span-4">
            <div class="rounded-xl border border-slate-200 bg-white p-6">
                ...
                <div class="space-y-3">
                    ...
                    <?php else: ?>
                        <form action="<?= url('/admin/app-store/themes/install') ?>" method="post">
                            <input type="hidden" name="csrf" value="<?= h(csrf_token()) ?>">
                            <input type="hidden" name="resource_id" value="<?= $themeId ?>">
                            <input type="hidden" name="return_to" value="<?= h($returnTo) ?>">
                        ...
                        <?php if ($isInstalled && !$isActive && $localTheme && !empty($localTheme['is_valid'])): ?>
                            <form action="<?= url('/admin/app-store/themes/activate') ?>" method="post">
                                <input type="hidden" name="csrf" value="<?= h(csrf_token()) ?>">
